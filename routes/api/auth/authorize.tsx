...
export const handler = async (req: Request, _ctx: HandlerContext): Promise<Response> => {
    ...
    const db = client.database("PurpleRoom");
    ...
    const check = new Sha1().update(decodeURI(password)).toString();
    const user: IUser | undefined = await users.findOne({ username: username, password: check });
